...
class AutomacaoGUI:
    def __init__(self):
        ...
        self.window = ctk.CTk()
        self.window.title("DomBot - Taxa GMS")
            ...
            for idx, (index, row) in enumerate(df.iloc[linha_inicial-1:].iterrows()):
                ...
                try:
                    log_msg = (f"Linha {index + 1} - Nº {row['Nº']} - "
                            f"EMPRESA: {row.get('EMPRESAS', 'N/A')}")
    ...
    def processar_linha(self, row, index):
        try:
            ...
            try:
                ...
                try:
                    self.log("Localizando itens de menu para Taxa GMS")
                    # Navegação por tab e preenchimento
                    self.log("Acessando pelo teclado")
                    send_keys('d')
                    time.sleep(0.3)
                    send_keys('d')
                    time.sleep(0.3)
                ...
                try:
                    ...
                    try:
                        ...
                        # Verificar se a janela de erro apareceu
                        ...
                        if error_window.exists():
                            self.log("Janela de erro detectada, pressionando Enter")
                            send_keys('{ENTER}')
                            time.sleep(1)  # Pausa após pressionar Enter
